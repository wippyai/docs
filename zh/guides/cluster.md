...
共识核心是**无磁盘**的：Raft 日志和快照仅存储在内存中，无需预置数据目录。重启后，节点重新加入 gossip 并从对等节点重放状态。这有意消除了磁盘 Raft 带来的持久化与 quorum 故障模式，与内存协调系统（Erlang global、Akka distributed data）的模型一致。权衡之处在于：集群的持久性依赖于活跃 quorum，而非磁盘——参见[恢复](#恢复与故障模式)。
...
- **Eventual** — 允许短暂过期窗口的高并发存在/会话名称。可扩展到非常大的名称数量。当两个源注册同一名称时，冲突解决机制选取胜者，落败进程收到取消事件（`process.event.CANCEL`），原因为 `name revoked: <name>`；它继续运行并可重新注册。节点离开时释放名称。
...
进程组是以 Erlang `pg` 为模型的集群感知发布/订阅与成员资格机制。进程加入命名组；向该组的广播会到达所有节点的所有成员。组基于 gossip，最终一致——独立于 Raft——因此即使共识核心正在收敛时也能正常工作。
...
由于共识核心无磁盘，持久性来自活跃 quorum 而非磁盘。实践规则：
