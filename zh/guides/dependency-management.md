...
- 完整的 `wippy update` 会从每个模块声明的范围求解；定向更新与启动时修复则会保留仍然满足所有活动范围的已固定版本。
- **根参数优先于传递参数**：当你的应用和某个依赖都绑定同一个需求时，你的 `ns.dependency` 上的参数优先。版本范围永远不会被覆盖；每个声明都会加入交集。
- 由多个根 `ns.dependency` 条目声明的组件由其中之一控制 — 既有声明先于新声明，带参数的先于不带参数的，平局时取条目 ID 最小者 — 其余的折叠为对它的引用。若重复声明的参数与控制声明不一致，则以冲突错误被拒绝；请改为更新已有的依赖。
