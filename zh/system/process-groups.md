...
进程组让进程加入命名组并接收发给组的广播，成员资格在集群的每个节点上追踪。模型遵循 Erlang/OTP `pg`：组在首次加入时创建，进程可以属于多个组（也可以多次加入同一组），没有中央注册表——每个节点维护状态并通过 gossip 与对等节点协调。
...
**加入/离开传播。** 本地加入或离开操作先应用到循环，然后扇出到存活成员对等节点与任何先前发现的远程节点的并集。向该并集而非仅 gossip 发现的对等节点发送，确保新加入或尚未收敛的节点仍能收到变更。
