...
Opções de `add*`: `{ method = "store"|"deflate", mode, modified }`. O escritor de zip faz streaming para escritores não-seekable usando data descriptors, então escrever em um stream de resposta funciona.
