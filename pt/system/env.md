...
A resolução acontece apenas no momento do decode: a entrada armazenada no registro mantém os placeholders crus, então segredos resolvidos nunca aparecem em resultados de `registry.get` nem em estado persistido. Entradas que referenciam uma variável pelo ID da entrada (`${env:ns:name}`) são ordenadas automaticamente depois dessa variável no boot; uma referência pelo nome público não cria aresta de dependência.
