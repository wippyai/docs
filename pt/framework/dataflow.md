---
title: "Dataflow"
description: "Componha e execute workflows DAG com nós, roteamento, persistência, sinais, templates e APIs de cliente do Wippy Dataflow."
---
...
O módulo dataflow depende de `wippy/agent`, `wippy/llm`, `wippy/session`, `wippy/test` e `wippy/migration`; `wippy install` resolve essas dependências. Por padrão, ele usa `app:db` para persistência e `app:processes` para o serviço de wake. Forneça essas entradas ou sobrescreva os requisitos `target_db` e `process_host`. As migrações são executadas por `wippy/migration`.
...
Use `:with_input()` para dados externos do workflow e `:with_data()` para configuração, constantes e dados de referência compartilhados por vários nós. Dados estáticos usam otimização por referência: a primeira rota cria os dados e as posteriores criam referências leves.
...
Nesse grafo, os dois planejadores podem executar em paralelo. O erro de um planejador segue a mesma rota de sua saída bem-sucedida até o consolidador.

## Mesclagem de entradas

O formato da entrada depende dos discriminadores das rotas e de o nó definir `args`.
...
<note>
Nós com <code>args</code> ou <code>input_transform</code> no formato string não podem receber entradas com o discriminador <code>"default"</code>. Use discriminadores nomeados com <code>:to(target, "input_key")</code>.
</note>
...
| `active_traits` | array | Sobrescreve os traits ativos do agente selecionado; um array vazio os desabilita neste nó |
| `active_tools` | array | Sobrescreve as ferramentas ativas do agente selecionado; um array vazio as desabilita neste nó |
...
A função de ciclo recebe em cada iteração; após a primeira, dados duráveis devem ser lidos de `state`:
...
| `max_iterations` | number | Iterações máximas (padrão: 100) |
| `initial_state` | table | Estado inicial (padrão: `{}`) |
| `continue_condition` | string | Expressão: continuar enquanto verdadeira |
| `inputs` | table | Requisitos de entrada |
| `context` | table | Contexto de execução passado à função de ciclo |
| `input_transform` | string/table | Transforma as entradas antes de o ciclo recebê-las |
| `metadata` | table | Metadados do nó |
...
Processa itens de um array por um template reutilizável:
...
| `batch_size` | number | Inteiro positivo até 1000; máximo de itens em execução (padrão: 1) |
| `scheduling` | string | `"batch"` (padrão) espera a onda inteira; `"rolling"` repõe vagas concluídas e exige `on_error = "continue"` |
| `on_error` | string | `"continue"` (padrão) ou `"fail_fast"`; `"collect_errors"` é um alias de compatibilidade para `"continue"` |
...
| `inputs` | table | Requisitos de entrada |
| `input_transform` | string/table | Transforma as entradas antes do processamento paralelo |
| `metadata` | table | Metadados do nó |
...
Pausa um nó até que um sinal externo chegue. Isso atende a aprovações humanas, eventos externos e workflows em estágios:
...
- `client:signal()` armazena dados omitidos, `nil` ou `false` como `{}`. Esse objeto vazio, assim como valores preservados como `0` e `""`, satisfaz o yield.
...
- `client:signal()` enfileira o sinal de forma durável e solicita a ativação do workflow. Se ele chegar antes de o nó atingir o yield, será entregue quando o yield for rastreado; não é necessário chamar `:start()` separadamente.
...
- Quando vários yields ativos compartilham o mesmo `signal_id`, um deles recebe os dados; qual deles não é definido. Use IDs exclusivos quando o destinatário importar.
- Omitir `signal_id` gera um UUID v7 que o builder não retorna. Defina um ID explícito e estável para sinais enviados pela API do cliente.
...
Nós signal participam de qualquer topologia. Adicione o binding do cliente ao lado do import de `flow` mostrado acima:
...
Os dados armazenados do sinal são expostos como saída do nó. Nós downstream recebem o payload enviado, exceto quando dados omitidos, `nil` ou `false` são normalizados para `{}`.
...
| `inputs` | table | Requisitos de entrada |
| `input_transform` | string/table | Transforma as entradas antes da junção |
| `metadata` | table | Metadados do nó |

## Templates

Templates definem sub-workflows reutilizáveis. Crie um com `flow.template()` e insira-o com `:use()`:
...
<note>
Uma função que precisa gerar um workflow filho deve retornar <code>flow.create():run()</code>. Outras funções do dataflow podem retornar resultados comuns.
</note>

## Síncrono vs Assíncrono

`:run()` executa de forma síncrona. Normalmente retorna a saída terminal, mas uma espera durável pode passivar a execução; nesse caso, o resultado inclui `pending = true`, `passivated = true` e o ID do workflow.
...
| `:revive(dataflow_id)` | Solicitar ativação de um workflow não terminal |
...
| `waiting` | Passivado enquanto aguarda um evento durável, como um sinal |
| `completed` | Concluído com sucesso |
| `failed` | Falhou |
| `cancelled` | Cancelado pelo usuário |
| `terminated` | Terminado à força |

Os nós têm um ciclo de vida separado. As transições atuais usam `template`, `pending`, `running`, `waiting`, `completed`, `failed` e `cancelled`. `ready` é aceito como status de ativação de um workflow carregado; `paused`, `skipped` e `terminated` no nível do nó são valores de compatibilidade reconhecidos, mas não são gravados pelas transições atuais.
...
O compilador valida o grafo do workflow antes da execução:
...
- Nós com `args` ou `input_transform` no formato string não podem receber entradas com o discriminador `"default"`
...
- [LLM](framework/llm.md) — Interface de modelos usada pelos agentes
