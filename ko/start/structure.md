...
<note>
YAML 정의는 시작 시 레지스트리에 로드됩니다. 레지스트리가 진실의 소스입니다. YAML 파일은 이를 채우는 한 가지 방법입니다. 엔트리는 다른 소스에서 오거나 프로그래밍 방식으로 생성될 수도 있습니다.
</note>
...
의미적 구분에는 점(`.`)을, 단어 구분에는 밑줄(`_`)을 사용합니다:
...
<tip>
패턴: <code>base_name.variant</code> - 점은 의미적 부분을 구분하고, 밑줄은 부분 내의 단어를 구분합니다.
</tip>
...
`entries` 배열의 각 엔트리. 속성은 루트 레벨에 있습니다 (`data:` 래퍼 없음):
...
규칙: `meta.title`과 `meta.comment`는 관리 UI에서 잘 렌더링됩니다.
