...
**컨텍스트는 선언되는 것이지 허공에서 상속되지 않습니다.** 함수는 호출자의 액터와 스코프를 상속합니다. 스폰된 프로세스는 그렇지 않습니다. 자신의 항목에 있는 `security:` 블록에서 컨텍스트를 가져오며, 이 블록은 액터를 지정하고 정책과 정책 그룹을 레지스트리 ID로 나열합니다. 해석은 원자적입니다. 지정된 정책이나 그룹 중 하나라도 없으면 부분적인 스코프로 진행하지 않고 스폰이 실패합니다. CLI 명령은 추가로 `meta.command.security`를 선언할 수 있으며, 이는 운영자가 직접 명령을 시작한 신뢰된 실행 경로에서만 적용됩니다.
