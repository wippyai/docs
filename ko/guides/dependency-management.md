...
- **루트 선언이 전이적 선언보다 우선합니다**: 앱과 의존성이 같은 모듈이나 요구사항을 함께 가져오는 경우, 앱의 선언이 우선합니다.
...
잠금 파일의 모든 모듈은 아티팩트 다이제스트를 가지며, 다이제스트가 없는 모듈은 아예 설치할 수 없습니다.

다운로드는 스테이징됩니다: 팩은 최종 위치 옆의 임시 파일에 기록되고, `wippy.lock`에 고정된 다이제스트와 허브가 다운로드 URL과 함께 제공한 다이제스트(및 제공된 크기) 양쪽에 대해 검증된 뒤에야 제자리로 rename됩니다. 검증에 실패한 스테이징 파일은 삭제됩니다.

다이제스트 불일치는 재시도 불가능한 강한 실패이며 — `PermissionDenied`, "module integrity verification failed" — 설치 시점과 부팅 시점 모두에서 동일하게 발생합니다. 부팅 시에는 엔트리가 로드되기 전에 이미 벤더링된 팩이 다시 검증됩니다. 어떤 것도 재시도하거나, 불일치를 덮어쓰며 다시 다운로드하거나, 제공된 콘텐츠로 폴백하지 않습니다.
