...
합의 코어는 **디스크 없는** 방식입니다: Raft 로그와 스냅샷은 메모리에만 있으므로 프로비저닝할 데이터 디렉토리가 없습니다. 재시작 시 노드가 gossip에 다시 참여하고 피어로부터 상태를 재생합니다. 이는 의도적으로 온디스크 Raft가 도입하는 지속성 대 쿼럼 장애 모드를 제거하고, 메모리 내 조율 시스템(Erlang global, Akka distributed data)의 모델과 일치합니다. 트레이드오프: 클러스터의 내구성은 디스크가 아닌 살아있는 쿼럼에서 옵니다 — [복구](#복구-및-장애-모드)를 참조하세요.
