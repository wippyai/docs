...
`funcs`를 임포트하는 게스트는 레지스트리를 다시 호출할 수 있습니다. 각 호출은 대상 ID에 대해 `funcs.call`로 정책 검사를 거치므로, 도달 가능한 범위는 정확히 상속된 스코프가 이미 허용하는 것뿐입니다. 소켓 다이얼도 같은 방식으로 [네트워크 서비스](system/network.md)가 `socket.*` 권한에 대해 인가합니다.
