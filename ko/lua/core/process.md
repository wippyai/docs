...
| `upgradable` | boolean | 프로세스의 코드가 무효화될 때 OUTDATED 이벤트를 받도록 옵트인 |
...
| `process.event.OUTDATED` | 프로세스의 코드 또는 임포트된 의존성이 레지스트리에서 변경됨 |
...
대상 프로세스를 빌더의 컨텍스트, 액터, 스코프 아래에서 동기적으로 실행하고 그 결과 값을 반환합니다 — 모듈 수준 `process.exec`의 바운드 대응물입니다. 지연된 워커는 `with_actor`/`with_scope`로 소유자의 신원을 재구성하여 그를 대신해 실행할 수 있습니다.
