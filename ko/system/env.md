---
title: "환경 시스템"
description: "메모리, 파일, 운영 체제, 정적 값 또는 스토리지 라우터를 백엔드로 사용하는 환경 변수를 정의합니다."
---
...
| 속성 | 타입 | 설명 |
|----------|------|-------------|
| `storages` | array | 필수이며 비어 있지 않은 정렬된 스토리지 참조 목록 |
...
| 속성 | 타입 | 설명 |
|----------|------|-------------|
| `variable` | string | 선택적 공개 변수 이름 |
| `storage` | string | 필수 스토리지 참조 (`namespace:name`) |
...
항상 문자열을 생성합니다. 변수 자체의 `default`가 플레이스홀더의 인라인
`|default`보다 먼저 적용됩니다. 아무것도 해석되지 않고 기본값도 없는 참조는
디코딩에 실패합니다.
