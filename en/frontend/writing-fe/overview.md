...
**Quick rule:** if it needs `vue-router`, a dedicated URL, or owns the full viewport — it is a page app. If it is embeddable, reusable, and self-contained — it is a web component.

When in doubt, start with a web component. It is easier to promote to a page app later than the reverse.

## What to read next

Build a page app:
1. [Page App](./page-app.md) — scaffold, `package.json` wippy block, Vite config, bootstrap sequence, router sync
...
4. [Theming](./theming.md) → [Theming: Page Apps](./page-app-theming.md) — CSS variable catalog, then how to receive it via proxy injections
...
4. [Theming](./theming.md) → [Theming: Web Components](./wc-theming.md) — CSS variable catalog, then how to receive it across the shadow DOM boundary
