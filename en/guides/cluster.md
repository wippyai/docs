...
Names are released automatically: Local on process exit; Consistent and Strong on process exit (via topology monitoring) and on node departure; Eventual on node departure. Resolution for messaging (`process.send`, `process.terminate`, and similar) consults the planes in order — Consistent, then Eventual, then Local — so a Consistent name shadows an Eventual one with the same string.
