---
title: "부트로더"
description: "wippy/bootloader 모듈은 시작 시 정의된 순서로 부트로더 함수를 탐색하고 실행하여 애플리케이션 초기화를 조율합니다. 다른 프레임워크 모듈(마이그레이션, 암호화, 인덱스 갱신)은 자체 초기화 단계를 실행하기 위해 부트로더를 등록합니다."
---
...
`wippy/migration`에서 제공됩니다. `meta.type: migration`을 가진 모든 엔트리를 탐색하고, `meta.target_db`별로 그룹화한 다음, 대기 중인 것을 적용합니다. [마이그레이션](framework/migration.md)을 참조하세요.
