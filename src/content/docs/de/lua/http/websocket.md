...
Die `channel()`-Methode gibt einen Channel zum Empfangen von Nachrichten zurück. `receive()` ist ein Alias für `channel()`. Funktioniert mit `channel.select` für Multiplexing.
