---
title: "プロセスモデル"
description: "Wippyは分離されたプロセスでコードを実行します。これはメッセージパッシングを通じて通信する軽量なステートマシンです。このアクターモデルアプローチにより、共有状態に起因するバグが排除され、並行プログラミングが予測可能になります。"
---
...
プロセスはLuaに限定されません。ランタイムは `process.wasm` 種別を介してWebAssemblyモジュールを既にサポートしており、アーキテクチャは任意のステートマシン実装を受け入れます。

<warning>
プロセスは軽量ですが無料ではありません。各プロセスは状態、受信ボックス、スケジューラの管理情報のための小さなベースラインコストを持ち、動的アロケーションが実行中にそのフットプリントを増大させます。
</warning>
