---
title: "ワークフロー"
description: "ワークフローはアクティビティをオーケストレーションし、障害や再起動をまたいで状態を維持する永続的な関数です。workflow.luaエントリ種別を使用して定義されます。"
---
...
結果をインラインで待つ場合の子ワークフロー実行の最も簡単な方法です。
...
子ワークフローがエラーを返した場合、EXITイベントに含まれます:
...
子ワークフローからのエラー（`process.exec`またはEXITイベント経由）は同じメタデータを持ちます:
