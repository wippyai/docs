...
### Требовать парсимые URL

Отклонять URL, которые не парсятся корректно. При `true` URL-атрибуты, которые HTML-санитайзер не может распарсить, удаляются вместо того чтобы пропускаться.
...
| Параметр | Тип | Описание |
|----------|-----|----------|
| `require` | boolean | Требовать, чтобы URL были парсимыми |
