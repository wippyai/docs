...
|----------|-----|----------|
| `...` | any | Переменное количество значений для записи (приводятся к строке) |
...
Включить или отключить raw-режим терминала (отключает построчную буферизацию и эхо):

```lua
local ok, err = io.raw(true)   -- включить
local ok, err = io.raw(false)  -- отключить
```

| Параметр | Тип | Описание |
|----------|-----|----------|
| `enable` | boolean | `true` для включения, `false` для отключения (по умолчанию: `true`) |

**Возвращает:** `boolean, error`

Raw-режим использует подсчёт ссылок — каждому `io.raw(true)` должен соответствовать `io.raw(false)`. Терминал автоматически возвращается в нормальный режим при выходе процесса.
