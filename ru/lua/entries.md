...
Каждый тип имеет предкомпилированный аналог в виде байткода (`function.lua.bc`, `library.lua.bc`, `process.lua.bc`, `workflow.lua.bc`), который создаётся командой `wippy pack --bytecode '**'` (или по шаблону вида `--bytecode 'app:**'`). Авторы пишут `.lua`-записи; типы байткода создаются при упаковке с этим флагом.
...
- name: handler
  ...
  method: main
  pool:
    type: adaptive    # явно; опустите для автовыбора (lazy)
    max_size: 16      # предел эластичного роста
```

| Поле | Пулы | Описание |
|------|------|----------|
| `type` | все | Реализация планировщика (см. таблицу ниже) |
| `workers` | static | Число рабочих потоков (при отсутствии берётся `size`, затем 8) |
| `size` | static | Число воркеров, когда `workers` не задан; при опущенном `type` `size` без `max_size` выбирает inline-пул |
| `buffer` | static | Размер очереди задач (по умолчанию `workers * 64`) |
| `max_size` | lazy, adaptive | Верхний предел эластичного роста (по умолчанию 16; 100, когда `type` опущен) |
...
| `adaptive` | Самомасштабирующийся пул — растёт под нагрузкой, сжимается в простое. |

Когда `type` опущен, пул выбирается автоматически по остальным полям: по умолчанию lazy, static — если задан `workers`, inline — если задан только `size`.
