...
| `size` | static | Число воркеров, когда `workers` не задан; также склоняет автовыбор к статическому пулу |
...
Когда `type` опущен, пул выбирается автоматически по остальным полям: по умолчанию lazy, static — если задан `workers`.
