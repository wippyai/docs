...
| `options.images` | table[] | Полный набор размещений сохраняемых изображений для кадра |
...
Записывает запрос к буферу обмена OSC 52 на физическом surface. Текст должен
быть корректным UTF-8 и занимать не более 65 536 байт. Успех означает, что
вывод терминала принял запрос; политика терминала всё равно может его
проигнорировать. Виртуальные surface возвращают ошибку неподдерживаемой
операции; API не предоставляет чтение буфера обмена или подтверждение.

**Возвращает:** `boolean, error`

## Сохраняемые изображения

Импортируйте PNG в ограниченное хранилище среды исполнения, затем поместите
его хэндл в полный кадр surface:
...
| `page` | table | нет | Непрозрачные значения цветов переднего и заднего плана по умолчанию в формате `#RRGGBB` |
...
| `images` | table[] | Метаданные размещений сохраняемых изображений |
| `layers` | table[] | Упорядоченные слои представления |
| `images_omitted` | boolean | Ресурсы изображений существуют, но этот обычный снимок их не сохраняет |
...
|----------|--------|----------|
| `tty.mount` | Хэндл viewport владельца | Выдать привязанный к процессу mount |
| `tty.observe` | Хэндл viewport владельца | Читать снимки, обновления и capture |
| `tty.input` | Хэндл viewport владельца | Пересылать события ввода |
| `tty.resize` | Хэндл viewport владельца | Изменять размер viewport |
