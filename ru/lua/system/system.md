...
## Источники развёртывания

Подтаблица `system.source` читает нормализованную базовую линию развёртывания: набор записей, произведённый источниками, из которых было собрано приложение, до применения какой-либо истории реестра.
...
| Поле | Тип | Описание |
|------|-----|----------|
| `owners` | string[] | Владельцы источников, авторитетные над записями базовой линии |
| `entries` | table[] | Записи базовой линии с `id`, `kind`, `meta`, `data` |

`owners` отсортирован так, что владелец приложения идёт первым, а остальные владельцы — по алфавиту. Владелец приложения — строка `"application"`.
...
Загрузка берётся из одного стабильного поколения источников, поэтому записи и владельцы всегда описывают одну и ту же базовую линию. Пути файловой системы за каждым источником приватны для среды выполнения и не раскрываются; неудачная загрузка сообщает об общей внутренней ошибке, а не выдаёт нижележащий путь.

**Разрешение:** `system.read` на `sources`
...
| `system.read` | `sources` | Загрузка базовой линии источников развёртывания |
...
|---------|------|------------|
| Доступ запрещён (`system.source.load`, `system.lock.*`) | `errors.PERMISSION_DENIED` | нет |
| Доступ запрещён (все остальные вызовы) | `errors.INVALID` | нет |
...
| Служба блокировок недоступна (нет Raft на этой ноде) | `errors.INTERNAL` | нет |
