---
title: "CDC"
description: "Подписка на потоки Change Data Capture из источников db.cdc.postgres и db.cdc.sqlite. Позволяет перечислить настроенные источники, открыть поток и…"
---
...
Поток получает снимок, если либо `opts.snapshot` равно true, либо в записи источника задано поле `snapshot`; строки снимка приходят первыми с `op = "snapshot"`, затем поток без разрыва продолжается живыми изменениями. `opts.after` зарезервирован для драйверов, возобновляющих работу с курсора — все поставляемые сегодня драйверы возвращают на него `errors.INVALID` ("cdc operation is not supported by this source"), включая `db.cdc.postgres`, когда он сообщает о `capture_resume`.
...
| `before` | Состояние строки до изменения (`update`, `delete`). Полный образ строки гарантирован, только когда источник имеет возможность `before_images`; `db.cdc.postgres` заполняет его тем старым кортежем, который несёт WAL, чем управляет `REPLICA IDENTITY` таблицы |
...
| `streaming` | `db.cdc.sqlite`: работает ли источник; `db.cdc.postgres`: настройка протокола `streaming` в записи |
...
if not info.capabilities.before_images then
    -- before не гарантирует полный образ строки; храните собственное последнее известное состояние
end
```
