...
`wb:rows(sheet)` открывает потоковый курсор по одному листу. Лист декодируется инкрементально с постоянным потреблением памяти, в отличие от `get_rows`, который материализует весь лист:
...
| Метод | Описание |
|-------|----------|
| `cursor:read(n?)` | Прочитать следующую партию до `n` строк (по умолчанию 1, максимум 10000). Возвращает `string[][], error`; `nil, nil` в конце листа |
| `cursor:close()` | Освободить курсор (идемпотентно; курсоры также закрываются вместе с книгой) |

Значения ячеек форматируются так же, как в `get_rows`. Пустые строки возвращаются пустыми таблицами, а завершающие пустые строки сохраняются, а не отбрасываются. После конца листа или ошибки последующие чтения продолжают возвращать то же состояние.
...
- [Файловая система](lua/storage/filesystem.md) — файловые операции для чтения/записи Excel-файлов
