...
`unmarshal()` всегда транскодирует в формат Lua и возвращает полученное значение Lua. В отличие от `data()`, который возвращает исходное значение (потенциально Go-объект для не-Lua форматов), `unmarshal()` гарантирует полностью декодированное значение Lua.
