...
Принудительное декодирование payload в значение Lua, независимо от исходного формата:

```lua
local data, err = p:unmarshal()
```

`unmarshal()` ведёт себя как `data()`: оба транскодируют не-Lua payload в формат Lua и возвращают полученное значение Lua. Отличие лишь в том, что `unmarshal()` возвращает ошибку, если транскодированные данные не являются корректным значением Lua, тогда как `data()` возвращает `nil`.
