...
| `upgradable` | boolean | Включить получение событий OUTDATED, когда код процесса инвалидируется |
...
| `process.event.OUTDATED` | Код процесса или импортированная зависимость изменились в реестре |
...
OUTDATED доставляется только процессам, включившим его через `process.set_options({upgradable = true})`; остальные процессы его никогда не видят. Несколько инвалидаций сливаются в одно ожидающее событие с объединением `sources`. Ожидаемая реакция — горячая замена через [`process.upgrade`](#process-upgrade).
...
## Обновление процесса {#process-upgrade}
...
Синхронно запускает целевой процесс под контекстом, актором и областью билдера и возвращает его результат — ограниченный аналог `process.exec` уровня модуля. Отложенный воркер может восстановить идентичность владельца через `with_actor`/`with_scope` и выполнить процесс от его имени.
