...
- `update` и `delete` работают только с записями, созданными этим владельцем.
...
Всё это проявляется как `errors.CONFLICT` или `errors.INVALID`, и ни одна из этих ошибок не повторяема: повторяемо только несовпадение поколений выше.
