---
title: "Контракты"
description: "Вызов сервисов через типизированные контракты. Обращение к удалённым API, workflow и функциям с валидацией схем и поддержкой асинхронного выполнения."
---
...
Без явных `with_actor`/`with_scope` открытый контракт наследует ambient-актора и область вызывающего. Когда они заданы, они распространяются на привязанные функции реализации — каждый вызов метода экземпляра выполняется под этой идентичностью.
