---
title: "Lua-среда"
description: "Основная среда вычислений Wippy, оптимизированная для I/O-нагруженных задач и бизнес-логики. Код работает в изолированных процессах, общающихся через…"
---
...
Всегда доступны без `require` и не требуют указания в `modules:`:

- `process` — создание процессов, отправка сообщений, мониторинг и связывание
- `channel` — каналы в стиле Go
- `payload` — входной payload записи
- `print`, `subscribe`, `unsubscribe` — логирование и pub/sub
- `os`, `table`, `math`, `string`, `coroutine`, `errors` — стандартные библиотеки

## Модули

Всё остальное загружается через `require()` и должно быть указано в allowlist `modules:` записи:
