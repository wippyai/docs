...
Каждый вызов записывает переданные байты и возвращает управление. Метода, закрывающего stdin, нет: он остаётся открытым на всё время жизни процесса, поэтому команда, читающая до конца ввода, например `sort`, никогда не видит EOF и завершается только когда процессу отправлен сигнал или он закрыт. Выбирайте команду, которая прекращает чтение сама, как `head -n 3`, или запускайте команду, которой нужен EOF, за shell-конвейером, поставляющим ей ввод.
