...
description: "Выполнение кода во время работы приложения в изолированном окружении с контролем доступа к модулям."
---
...
### Привилегированные импорты

Импорту можно предоставить модули, которые сам вычисляемый код не видит. Используйте табличную форму с `id` и `modules`:
...
Библиотека `pricing` выполняется в собственном изолированном окружении, где доступен `funcs`; вычисляемый исходный код не может ни подключить `funcs`, ни обратиться к нему напрямую. Чтобы предоставить модуль импорту, вызывающий должен обладать разрешением `eval.module` на этот модуль — возможности нельзя делегировать сверх того, что разрешено самому вызывающему.
...
Таблица опций принимает те же поля `modules` и `imports`, что и `runner.run`, и применяются те же проверки разрешений `eval.module` и `eval.import`. Скомпилированная программа носит информационный характер; для выполнения вызовите `runner.run` с исходным кодом и методом.
...
- `eval.module` — для каждого модуля в белом списке и для каждого модуля, предоставленного привилегированному импорту
...
Скомпилированные программы кэшируются в LRU с ключом по исходному коду, методу, модулям и разрешённым классам — повторные запуски идентичного кода пропускают перекомпиляцию. Импорты и контекст привязываются во время выполнения и не влияют на ключ кэша.
