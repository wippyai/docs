...
| `overlay_network` | string | Маршрутизировать через [сетевой оверлей](system/network.md) — registry ID записи `network.socks5` / `network.tailscale` / `network.i2p` |
...
|--------------|------------|----------|
| `read(n?)` | string, error | Читает до `n` байт (по умолчанию: буфер реализации) |
| `close()` | boolean, error | Закрывает поток |

`resp.stream` — это полноценный объект [stream](lua/core/stream.md): `seek`, `stat` и `scanner` также доступны.
...
Непубличные IP-диапазоны заблокированы по умолчанию. Доступ требует разрешения `http_client.private_ip` на адресе:

- loopback, приватные (10.x, 172.16-31.x, 192.168.x), link-local unicast и multicast, а также неуказанный адрес
- carrier-grade NAT `100.64.0.0/10`, `192.0.0.0/24`, multicast `224.0.0.0/4`, зарезервированный `240.0.0.0/4`
- диапазоны для документации и бенчмарков `192.0.2.0/24`, `198.18.0.0/15`, `198.51.100.0/24`, `203.0.113.0/24`, `2001:db8::/32`
...
Проверка выполняется в момент подключения, а не по строке URL, и охватывает каждый адрес, в который разрешается хост. Имя хоста, разрешающееся в несколько адресов, проверяется адрес за адресом: запрещённый адрес пропускается и пробуется следующий, а запрос падает, только когда все кандидаты запрещены или недостижимы. Поэтому публичное имя хоста, разрешающееся в приватный адрес, блокируется ровно так же, как литерал приватного IP.

### Редиректы

Выполняется до девяти редиректов; десятый падает с `stopped after 10 redirects` — счётчик включает исходный запрос.

Каждый переход авторизуется отдельно. Перед следованием за редиректом клиент вычисляет `http_client.request` для целевого URL и применяет к нему проверку приватных IP, поэтому разрешённый URL нельзя использовать, чтобы редиректом добраться до запрещённого. Переход, не прошедший любую из проверок, прерывает запрос.
