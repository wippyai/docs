---
title: "Сетевые оверлеи"
description: "Маршрутизация исходящего трафика и привязка слушателей через оверлейные сети (SOCKS5-прокси, Tor, Tailscale mesh, I2P). Выбор оверлея включается по…"
---
...
Выбор оверлея не ограничен границами Lua. Подключения, выполняемые через сетевой сервис среды выполнения — WASM-хост [`socket`](wasm/hosts.md#socket) и диспетчер `wasi:sockets` — читают оверлей из фрейма и маршрутизируют через него, независимо от того, задан ли он через `with_options`, через `meta.options.network` на записи или через `network_service.default_network`.

Гейт приватных IP на этом пути ведёт себя иначе. Прямое подключение разрешает цель и проверяет каждый полученный адрес по `socket.private_ip`. При выбранном оверлее проверяется только литеральный IP-адрес в цели; имена хостов передаются оверлею для разрешения, поэтому локальный резолвер не опрашивается и проверка того, что он вернул бы, не выполняется.

Когда оверлей выбран, но контекст не несёт сетевого реестра, подключение падает с `network "<id>" selected without a network registry`.
...
| `socket.connect` | `host:port` | Любое исходящее подключение через сетевой сервис |
| `socket.listen` | `host:port` | Привязка TCP-слушателя или UDP-сокета через сетевой сервис |
| `socket.resolve` | Имя хоста | Разрешение DNS через сетевой сервис |
| `socket.private_ip` | IP-адрес | Обращение к loopback, приватному, link-local или неуказанному адресу |
...
Разрешения `socket.*` проверяются самим сетевым сервисом. `socket.connect`, `socket.listen` и `socket.resolve` проверяются до какой-либо маршрутизации через оверлей, поэтому применяются одинаково к трафику в clearnet и через оверлей; `socket.private_ip` сужается до литеральных адресов, как только выбран оверлей, — как описано в разделе [Сырые подключения](system/network.md#raw-dials).
...
- [Хост-функции](wasm/hosts.md) - Импорты сокетов в WASM
