...
| `token_key` | нет | Ключ подписи HMAC-SHA256 (прямое значение или `${env:NAME}` для получения из [реестра окружения](system/env.md)) |

В продакшене используйте `token_key: ${env:NAME}`, чтобы не хранить секреты в записях. Устаревшая директива `token_key_env` разрешается тем же способом, но объявлена устаревшей; предпочитайте `${env:NAME}`.
...
И функции, и порождённые процессы наследуют контекст безопасности вызывающего. Порождённый процесс начинает на фрейме, ответвлённом от фрейма породившего, который несёт актора и область породившего, а блок `security:` в его собственной записи изменяет этот унаследованный контекст. Когда запись не объявляет блок, процесс сохраняет актора и область породившего без изменений; породивший, у которого нет ни того, ни другого, даёт потомка без того и другого, что строгий режим запрещает. Объявленный блок, указывающий `actor`, заменяет унаследованного актора, а его `policies` и `groups` сливаются с унаследованной областью; блок, опускающий `actor`, сохраняет актора породившего, а опускающий и `policies`, и `groups` — сохраняет его область.
...
Каждый модуль в `wippy.lock` несёт дайджест артефакта. При загрузке среды скачивание проверяется и по дайджесту, закреплённому в lock-файле, и по дайджесту, отданному хабом, а уже вендорные пакеты перепроверяются по lock-файлу перед загрузкой; несовпадение — это неповторяемый сбой целостности, который не обходится: модуль не загружается. `wippy install` проверяет свежее скачивание только по дайджесту и размеру, отданным хабом, при несовпадении удаляет файл и завершается неудачей, а затем записывает отданный дайджест обратно в lock-файл, так что закреплённый дайджест установкой переустанавливается, а не принуждается; только пакеты, уже лежащие в вендорном каталоге, сверяются с дайджестом из lock-файла. Распакованные каталоги модулей несут собственные записанные дайджест и дайджест дерева и проверяются так же, поэтому изменённое вендорное дерево обнаруживается, а не принимается на веру. См. [Управление зависимостями](guides/dependency-management.md#integrity-verification).
...
4. **Подпись токенов** — всегда задавайте `token_key` из ссылки `${env:NAME}` в продакшене
