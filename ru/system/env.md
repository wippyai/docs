...
## Ссылки в конфигурации записей

Зарегистрированные переменные подставляются в конфигурацию записей плейсхолдерами `${env:NAME}`, где `NAME` — публичное имя переменной или её entry ID (например, `app.env:tls_cert`).
