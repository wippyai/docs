...
Разрешение происходит только на этапе декодирования: сохранённая запись реестра хранит сырые плейсхолдеры, поэтому разрешённые секреты никогда не появляются в результатах `registry.get` или в персистентном состоянии. Записи, ссылающиеся на `${env:...}`, при загрузке автоматически упорядочиваются после env-хранилищ и переменных, от которых они зависят.
