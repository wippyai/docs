...
description: "Делитесь переиспользуемым кодом в Wippy Hub."
---
...
3. Имейте право создавать модули в этой организации — первый `wippy publish` регистрирует модуль автоматически
...
| `version` | Нет | Семантическая версия; `--version` переопределяет её |
| `exclude` | Нет | Паттерны для отбрасывания: значения, содержащие `:`, — это ID записей, всё остальное — glob по файлам исходников |
| `embed` | Нет | Паттерны встраивания `fs.directory` по умолчанию, когда `--embed` не передан |
| `exclude_meta` | Нет | Карта поля метаданных в значения; записи, чьи метаданные совпадают, отбрасываются |
| `metadata` | Нет | Произвольные метаданные key/value, переносимые с опубликованным модулем |
| `publish.profiles` | Нет | Какие профили конфигурации поставлять в пакете (см. [Публикация профилей](#publishing-profiles)) |
| `publish.runtime` | Нет | Какие секции конфигурации среды выполнения поставлять как значения по умолчанию пакета; только `type: application` |

`exclude` разделяется по форме значения, а не отдельным полем. `_old/**`, `test/**` и `*.test.lua` фильтруют файлы исходников при их сборе; `acme.http:debug_handler` отключает запись реестра после декодирования записей. Сегмент `**` покрывает любое количество сегментов пути.
...
Публикация собирает пакет одинаково с `--dry-run` и без него, поэтому валидация покрывает всё, что произвела бы настоящая публикация:

- `organization` и `module` должны быть в нижнем регистре из букв и цифр с дефисами внутри, `version` должна быть semver, а `type` — одним из четырёх типов модуля.
- `publish.runtime` принадлежит приложению: объявление `source`, `sections` или `vars` под ним без `type: application` приводит к ошибке.
- Каждый ресурс, объявляющий `meta.artifact.format`, проверяется этим форматом. Некорректный артефакт падает здесь, а не у потребителя, а два артефакта, чьи выходы попали бы в пересекающиеся каталоги, отклоняются.
- Формат `node-package` дополнительно требует, чтобы `package.json` содержал семантическую `version`, **равную публикуемой версии модуля**, корректное `name` пакета и ни одного lifecycle-скрипта `preinstall`, `install`, `postinstall` или `prepare`.

Последнее правило и подводит при релизе: поднимайте `version` в `wippy.yaml` и в `package.json` артефакта вместе, иначе публикация остановится.
...
Модули с записями `fs.directory` (статические ассеты, шаблоны, публичные файлы) должны использовать `--embed`, чтобы включить их в опубликованный пакет. Без него запись `fs.directory` упаковывается без содержимого каталога.
