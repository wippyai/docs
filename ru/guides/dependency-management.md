...
- **Объявления корня выигрывают у транзитивных**: когда ваше приложение и зависимость тянут один и тот же модуль или требование, приоритет у вашего объявления. Запись зависимости с `meta.module` считается транзитивной, если явно не помечена как корневая — опубликованные приложения сохраняют объявленные в исходниках зависимости как корневые.
...
Ключи — это `org/module`, значения — каталоги (относительные пути разрешаются относительно каталога первого файла `--config`; путь должен существовать и быть каталогом). Установка замены в `null` отключает замену, унаследованную из более раннего слоя конфигурации или профиля. Замены также могут находиться внутри [профиля](guides/configuration.md#profiles), чтобы активироваться только с `--profile workspace`.
