...
Каждый модуль в lock-файле несёт дайджест артефакта, и модуль без него вообще не может быть установлен.

Загрузки промежуточные: пакет пишется во временный файл рядом с итоговым расположением, проверяется по дайджесту, закреплённому в `wippy.lock`, и по дайджесту, который хаб отдал вместе с URL загрузки (плюс отданный размер), и только затем переименовывается на место. Промежуточный файл, не прошедший проверку, удаляется.

Несовпадение дайджеста — жёсткий, неповторяемый сбой (`PermissionDenied`, "module integrity verification failed"), и он возникает одинаково при установке и при загрузке, где уже вендорные пакеты перепроверяются перед загрузкой записей. Ничто не повторяет попытку, не перезагружает поверх несовпадения и не откатывается на отданное содержимое.
