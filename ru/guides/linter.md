---
title: "Линтер"
description: "Wippy включает встроенный линтер, который выполняет проверку типов и статический анализ Lua-кода. Запустите его командой wippy lint."
---
...
Записи с байт-кодом хранят скомпилированный байт-код (fs/path/hash), а не исходный код, поэтому их нельзя разобрать или проверить на типы; линтер проверяет только Lua-записи с исходным кодом (их `.bc`-варианты пропускаются, хотя могут учитываться в общем количестве записей).
...
### Необъявленные require

Строковый литерал `require("name")`, чей модуль не указан ни в объявлениях `imports`/`modules` записи, ни среди ambient-встроенных, завершается ошибкой:

```
require("name") is not declared in _index.yaml imports or modules
```

Эта проверка выполняется всегда (она не скрыта за `--rules`) и сообщается как ошибка. Объявите модуль, чтобы её пройти:
...
Динамические require (`require(variable)`) не проверяются. Ambient-набор — модули, доступные без объявления, например `process` в исполняемых видах записей, — общий у линтера и среды выполнения, поэтому разрешение на этапе линтинга и на этапе выполнения не может разойтись.
...
| `--profile` | | | Применить профиль рабочего пространства из объединённой конфигурации среды исполнения (повторяемый, применяется по порядку) |
| `--set` | | | Переопределить значение объединённой конфигурации среды исполнения (`section.path=value`, повторяемый) |
