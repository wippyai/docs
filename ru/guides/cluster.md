---
title: "Кластер"
description: "По умолчанию Wippy работает как одна нода. Включение кластера превращает набор нод в единую координированную систему, разделяющую membership,…"
---
...
- **Реплицируемые KV-хранилища** — `store.kv.raft` (строгая) и `store.kv.crdt` (в конечном счёте) реплицируют KV-данные по нодам (см. [Store](system/store.md#cluster-kv-stores)).
...
Состояние Raft **по умолчанию персистентно на диске**: логи и снимки сохраняются в `cluster.raft.data_dir` (по умолчанию `~/.wippy/store`, в `_sys/raft`), и [`store.kv.raft`](system/store.md#cluster-kv-stores) реплицируется через то же ядро. Перезапускающаяся нода всё равно переприсоединяется к gossip и догоняет состояние от пиров, поэтому кластер также переносит потерю диска ноды; долговечность обеспечивается как живым кворумом, так и состоянием на диске. Нода работает бездисково только когда каталог данных не определяется (нет ни заданного пути, ни домашнего каталога) — см. [Восстановление](#восстановление-и-режимы-отказа).
...
`system.lock` — кластерное взаимное исключение, построенное на raft-линеаризуемой условной записи в общем key-value хранилище. Захват блокировки выполняет set-if-absent PID держателя по ключу `_sys:lock:<name>`; освобождение удаляет эту запись, если она всё ещё принадлежит вызывающему. Поскольку условная запись проходит через Raft (записи не с лидера перенаправляются лидеру), она линеаризуема, поэтому в кластере может существовать не более одного держателя.
...
Состояние Raft персистентно на диске, но основная долговечность кластера по-прежнему обеспечивается живым кворумом. Практические правила:
