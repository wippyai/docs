...
| `auto_ack` | false | - | Авто-подтверждение на уровне драйвера (autoAck в AMQP `Consume`; игнорируется драйвером memory) |
...
Обработчик может подтвердить сообщение сам через `queue.message()` и `msg:ack()` / `msg:nack()`; в этом случае консьюмер пропускает собственные ack/nack.
