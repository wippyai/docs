...
- Пул `inline` сериализует вызовы через один тёплый экземпляр и сбрасывает состояние выполнения между вызовами; политика удерживаемой памяти или ошибка выполнения может заменить экземпляр
