...
Сервер может терминировать TLS напрямую. Установите `tls.mode` в `manual` (предоставьте свой сертификат) или `auto` (сертификат предоставляется драйвером overlay-сети, например `network.tailscale`). Обычные clearnet-слушатели не поддерживают `auto`. Опустите `tls` или оставьте mode пустым, чтобы запустить обычный HTTP.

В режиме `auto` сервер не должен указывать `cert`/`key`/`cert_env`/`key_env` — их предоставляет драйвер сети.
