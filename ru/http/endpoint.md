...
Для обычного эндпоинта роутер дополнительно регистрирует обработчик `OPTIONS` на том же пути, поэтому CORS-middleware может ответить на preflight, не запуская эндпоинт. Эндпоинт с `*` такого обработчика не получает: он и так совпадает с `OPTIONS`, поэтому preflight-запросы доходят до самой функции эндпоинта, которая и должна на них отвечать.
