---
title: "Relay"
description: "Модуль wippy/relay предоставляет инфраструктуру WebSocket-ретрансляции с двухуровневой архитектурой хабов. Центральный хаб управляет хабами для каждого…"
---
...
| `max_connections_per_user` | нет | `5` | WebSocket-соединений на пользователя |
...
`status` плагина — одно из значений `"not_started"` (зарегистрирован, ни разу не запущен), `"pending"` (запуск в процессе), `"running"`, `"failed"` или `"stopped"`.
