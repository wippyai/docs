...
description: "Модуль wippy/test предоставляет BDD-фреймворк для тестирования с утверждениями, хуками жизненного цикла и моками."
---
...
Модуль автоматически регистрирует точку входа для тестов (команду с `use_case: test`). После установки `wippy test` обнаруживает и запускает все тестовые записи в проекте.
...
Раннер определяет, использует ли тест BDD-события или возвращает простое значение. Оба подхода работают с `wippy test`.
