---
title: "LLM"
description: "Модуль wippy/llm предоставляет унифицированный интерфейс для работы с большими языковыми моделями от различных провайдеров (OpenAI, Anthropic, Google,…"
---
...
| Параметр | Описание |
|----------|----------|
| `model` | Обязательный. Модель для проверки. |
| `provider_id` | Необязательный. Пропускает разрешение модели и обращается к конкретному провайдеру. |
...
- [Агенты](framework/agents.md) - Фреймворк агентов с инструментами, делегатами и памятью
