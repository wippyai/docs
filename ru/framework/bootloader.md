...
| `meta.order` | Нет | Порядок выполнения (по умолчанию `999`); меньшие выполняются первыми |
...
| `100` | Задачи уровня приложения (соглашение) |

Когда два загрузчика имеют одинаковый порядок, они выполняются в алфавитном порядке по своему полному ID записи.
