---
title: "Функции WASM"
description: "Функции WASM -- это записи реестра, которые выполняют WebAssembly-код. Доступны два вида записей: function.wat для встроенного WAT-исходника и…"
---
...
| Type | Описание |
|------|----------|
| `inline` | Сериализованный мьютексом. Синхронные и asyncified-вызовы повторно используют один тёплый экземпляр; политика удерживаемой памяти или ошибка выполнения может вызвать его замену. |
...
Установка `pool.worker_class` направляет функцию в выделенный пул воркеров, закреплённых за OS-потоками, вместо общих типов пулов выше (`type` игнорируется, когда задан этот параметр; конвенциональное имя: `wasm`):
...
Изоляция ядер включается для каждой среды выполнения отдельно в `.wippy.yaml`:
...
С включённой изоляцией планировщик акторов и закреплённые WASM-пулы работают на непересекающихся наборах CPU (`sched_setaffinity`, только Linux — другие платформы задают размер пулов, но не привязывают потоки). Долгие WASM-вызовы в этом случае не могут отнимать ресурсы у планирования акторов.
...
Блок `options.limits` ограничивает время выполнения функции, память тёплого воркера и сокеты, которые она может открывать:
