...
| `wippy:actor` | `wippy:actor/process@0.1.0` | component | Идентичность PID и ограниченный почтовый ящик актора |
...
Восемь профилей `wasi:*`, `funcs` и `wippy:actor` доступны только для компонентов: объявление любого из них на core-модуле делает запись невалидной. `wasi1` и `socket` предоставляют core-импорты.
