...
<note>
`process.wasm` разделяет структуру конфигурации с `function.wasm`, поэтому блок `pool` принимается схемой, но игнорируется — процессы выполняются под хостом процессов, а не под пулом функций.
</note>
