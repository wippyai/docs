...
| `process.wasm` | Актор WASM с состоянием и одним экземпляром модуля на PID |
...
Гость, импортирующий `funcs`, может вызывать обратно в реестр. Каждый вызов проверяется политиками как `funcs.call` по целевому ID, поэтому достижимое множество — ровно то, что уже разрешает унаследованная область. Подключения через сокеты авторизуются так же — [сетевым сервисом](system/network.md), по разрешениям `socket.*`.
