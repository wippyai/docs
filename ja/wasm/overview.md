...
`funcs`をインポートしたゲストはレジストリへコールバックできます。各呼び出しは対象IDに対する`funcs.call`としてポリシー検査されるため、到達可能な範囲は継承されたスコープが既に許可しているものと正確に一致します。ソケットのダイヤルも同様に、[ネットワークサービス](system/network.md)によって`socket.*`権限に対して認可されます。
