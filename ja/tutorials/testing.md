...
`wippy/test` フレームワークで Lua コードのテストを記述して実行します。これはアサーション、ライフサイクルフック、モックを備えた BDD スタイルのランナーで、`wippy run test` コマンドによって実行されます。
...
3. `wippy run test` によるグリーンなテスト実行。
...
`wippy run test` はすべてのケースが合格すると `0` で、失敗があれば `1` で終了するため、そのまま CI に組み込めます。
