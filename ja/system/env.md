...
解決はデコード時にのみ行われます。保存されるレジストリエントリは生のプレースホルダを保持するため、解決されたシークレットが`registry.get`の結果や永続化された状態に現れることはありません。`${env:...}`を参照するエントリは、ブート時に依存するenvストレージと変数の後に自動的に順序付けられます。
