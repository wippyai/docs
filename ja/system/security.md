...
関数は呼び出し元のセキュリティコンテキストを継承します。生成されたプロセスは継承しません。新しいフレーム上で開始し、そのコンテキストは自身のエントリの`security:`ブロックから得られます。エントリがブロックを宣言していない場合、プロセスはアクターもスコープも持たずに実行され、strictモードはこれを拒否します。宣言されたブロックが`actor`を省略した場合はスポーン元のアクターを継承し、`policies`と`groups`の両方を省略した場合はスポーン元のスコープを継承します。つまり「新規に開始する」とは「アンビエントな継承がない」という意味であり、「コンテキストを与えられない」という意味ではありません。
...
`wippy.lock` 内のすべてのモジュールはアーティファクトダイジェストを持ちます。ダウンロードはステージングされ、ロックに固定されたダイジェストとハブが提供したダイジェストの両方に対して検証されてから所定の位置に移されます。ベンダー化済みのパックはブート時に再検証されます。不一致はリトライも回避もされない`PermissionDenied`の失敗であり、モジュールはロードされません。展開されたモジュールディレクトリも自身の記録済みダイジェストとツリーダイジェストを持ち、同様にチェックされるため、変更されたベンダーツリーは信頼されずに検出されます。[依存関係管理](guides/dependency-management.md#integrity-verification)を参照してください。
