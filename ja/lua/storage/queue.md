...
分散キューからメッセージをパブリッシュおよびコンシューム。RabbitMQなどのAMQP互換ブローカーを含む複数のバックエンドをサポート。
...
コンシューマコンテキストでキューメッセージを処理する場合のみ利用可能。
...
キューコンシューマはペイロードを直接受け取るエントリポイントとして定義:
...
両方の権限がチェックされる: まず一般的な権限、次にキュー固有の権限。
