...
このガイドでは作成者側、つまりインターフェースの宣言方法と、値がエントリへ流れ込む仕組みを説明します。利用者側（ロックファイル、バージョン制約、`wippy add`/`update`）については[依存関係管理](./dependency-management.md)を参照してください。コンポーネントの内部構造については[アプリケーションアーキテクチャ](../concepts/architecture.md)を参照してください。
...
これにより、スーパーバイザが未登録のワーカーを参照している、テストフィクスチャがハーネス専用のストレージ ID を使っている、といった未解決の関係を検出できます。[スーパービジョン](./supervision.md)と[テスト](../framework/testing.md)フレームワークを参照してください。

## 関連項目

- [アプリケーションアーキテクチャ](../concepts/architecture.md) — コンポーネントの内部構造
- [依存関係管理](./dependency-management.md) — ロックファイル、バージョン、利用者側のワークフロー
- [モジュールの公開](./publishing.md) — コンポーネントを Hub に載せる
- [エントリ種別ガイド](./entry-kinds.md) — `ns.definition`、`ns.requirement`、`ns.dependency` のリファレンス
