...
ロックファイル内のすべてのモジュールはアーティファクトダイジェストを持ち、それを持たないモジュールはそもそもインストールできません。

ダウンロードはステージングされます。パックは最終的な配置先の隣にある一時ファイルへ書き込まれ、`wippy.lock` に固定されたダイジェストと、ハブがダウンロードURLとともに提供したダイジェスト（および提供されたサイズ）の両方に対して検証され、その後にのみ所定の位置へリネームされます。検証に失敗したステージングファイルは削除されます。

ダイジェストの不一致は、リトライ不可能な致命的失敗です — `PermissionDenied`、"module integrity verification failed" — インストール時と、ベンダー化済みのパックがエントリのロード前に再検証されるブート時の両方で、同じように発生します。リトライも、不一致を上書きする再ダウンロードも、提供されたコンテンツへのフォールバックも行われません。
